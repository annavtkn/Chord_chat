...
function sendMessage(text) {
    ...
    if (c) {
        if (c.status == E.STATUS.MPOTR) {
            ...
            chat.rooms[chat.currentRoom].offlineUsers.forEach((user) => {
                chord.get(`name${user.name}mail${user.mail}`)
                    .then((roomsBlock) => {
                        roomsBlock[chat.currentRoom].messages.push(msg);
                        chord.put(`name${user.name}mail${user.mail}`, roomsBlock);
                    });
            })
        }
    }
...
async function onDisconnectUser(roomsBlock) {
    chat.rooms[roomsBlock.currentRoom].offlineUsers.push({
        name: roomsBlock.name,
        mail: roomsBlock.mail,
    });
    await chord.put(`name${roomsBlock.name}mail${roomsBlock.mail}`, roomsBlock.rooms);
}
...
async function newRoom(rid, name, status) {
    if (!chat.getRoom(rid)) {
        ...
        $_.ee.addListener("disconnectUser", onDisconnectUser);
...
function newPeer(id) {
    chord.get(`id${id}`).then((name) => {
        ...
        recreatePreviousChats(name, id);

        GUI.showNotification(`Пользователь с id ${id} подключен`, 2000);
    });
}

function recreatePreviousChats(name, id) {
    chord.get(`name${chat.name}mail${chat.mail}`)
        .then((roomsBlock) => {
            console.log(`Find previous roomsBlock ${roomsBlock}`);
            if (roomsBlock) {
                for (let roomid in roomsBlock) {
                    let new_rid = BigInt.randBetween(0, 1e40).toString(16),
                        prev_name = roomsBlock[roomid].name;

                    newRoom(new_rid, prev_name);

                    chord.send(id, "-newRoom-", {rid: new_rid, name: prev_name});

                    GUI.buildRooms(chat.rooms);
                    GUI.showChat();

                    chat.currentRoom = new_rid;
                    roomsBlock[roomid].messages.forEach((msg) => {
                        msg.room = new_rid;
                        msg.type = E.MSG.UNENCRYPTED;

                        chat.addMessage(msg);
                        chord.publish(new_rid, "-message-", msg);
                    });
                }
            } else {
...
window.addEventListener("beforeunload", function beforeUnload(event) {
    for (let rid in chat.rooms) {
        ...
        chord.publish(rid, "-disconnectUser-", roomsBlock);
    }
    for (let rid in chat.rooms) quitRoom(rid);
});
...
ChordModule.prototype.getWRTCString(function (str) {
    ...
    chord.on("-disconnectUser-", function (roomsBlock) {
        EMITTERS[roomsBlock.currentRoom].ee.emitEvent("disconnectUser", [roomsBlock]);
    });
