...
function Node(opts, conn) {
	this.opts = Object.assign({
		...
		storeRecycleTimeout: 1000 * 60 * 1440 * 365,

		subscriptionExpiration: 1000 * 60 * 1440 * 365,

	}, opts)

	this.id = NodeId.hash(this.opts.id)
...
Node.prototype.get_groupPubKeys = function *() {

	var storedKeys = [];
	var targetId = this.id;

	var id = undefined;
	var method = 'get_groupPubKeys';
	var arg = {targetId, storedKeys};
	var ttl = undefined;

	return yield *this.pull(this.successorId, 'lookup', {id, method, arg, ttl});
...
Node.prototype.lookup = function *(id, method, arg, ttl) {
	...
	if (method === 'get_groupPubKeys') {
		let result = [];
		let keys = Object.keys(this.storage);
		for (let key of keys) {
			if(this.storage[key].val.type === "groupPubKey"){
				result.push(this.storage[key].val);
			}
		}

		arg.storedKeys = getUniqueListBy(arg.storedKeys.concat(result), 'roomId');
		
		if (this.id === arg.targetId){
			return arg.storedKeys;
		} else {
			return yield *this.pull(this.successorId, 'lookup', {id, method, arg, ttl});
		}
